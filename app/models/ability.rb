class Ability
  ...
  def initialize(user)
    ...
    can [:read, :sites_by_term, :search, :sites_info, :alerted_collections, :register_gateways, :upload_rrt, :import_rrt], Collection, :memberships => { :user_id => user.id }
    can [:read, :sites_by_term, :search, :sites_info, :alerted_collections, :register_gateways, :upload_rrt, :import_rrt], Collection, :public => true
