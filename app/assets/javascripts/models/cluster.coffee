$ ->
  ...
  rm.Cluster = (map, cluster) ->
    @map = map
    @setMap map
    @maxZoom = cluster.max_zoom
    @setData(cluster, false)

  rm.Cluster.prototype = new google.maps.OverlayView

  rm.Cluster.prototype.onAdd = ->
  ...
  rm.Cluster.prototype.setData = (cluster, draw = true) ->
    @position = new google.maps.LatLng(cluster.lat, cluster.lng)
    @setCount cluster.count
    @draw() if draw

  rm.Cluster.prototype.setCount = (count) ->
    @count = count
    @countDiv.innerText = (@count).toString() if @countDiv

  rm.Cluster.prototype.setActive = (draw = true) ->
    $(@div).removeClass('inactive')
    $(@shadow).removeClass('inactive')
    @draw() if draw

  rm.Cluster.prototype.setInactive = (draw = true) ->
